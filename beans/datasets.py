                ...
                for window_id in range(num_windows):
                    ...
                    for anon in data['annotations']:
                        ...
                        if anon['st'] <= st and ed <= anon['ed']:
                            y[label_id] = 1
                    # print('y: label in the audio window=', y) # too much outpouts due to loop inside "window"
                    self.ys.append(y)
                # print('ys: label in the audio=', self.ys) #jump out from each window and output entire file's labels
    ...
    def __getitem__(self, idx):
        ...
        if self.feature_type == 'waveform':
            ...
            x = _get_spectrogram(
                ...
                target_sample_rate=self.sample_rate)
            x = x[:, offset_st:offset_ed]
            ### gib.resnet==torch.Size([128, 400])
            # print('x=', x.shape)
            ### gib.resnet==0, 1, 2, ---,232
            # print('idx=', idx)
            ### gib.resnet==tensor([0., 0., 0.]), [0., 1., 0.], ..., [1., 0., 0.]
            # print('ys[idx]=', self.ys[idx])
